...
async function loadMyPlaylists(): Promise<void> {
  const section = document.getElementById('lib-playlists');
  if (!section) return;

  try {
    const res = await api.userPlaylist(String(state.userProfile?.userId || ''));
    const playlists: PlaylistInfo[] = (res.playlist || []).filter(
      (p: any) => p.userId === state.userProfile?.userId
    );
...
async function loadRecent(): Promise<void> {
  ...
  try {
    ...
    if (list.length > 0) {
      const listWrap = document.createElement('div');
      section.appendChild(listWrap);
      renderSongList(list.map((item: any) => mapBasicTrack(item.data)).slice(0, 10), {
        container: listWrap,
        onPlay: (_, i) => playTrack(mapBasicTrack(list[i].data), list.map((item: any) => mapBasicTrack(item.data))),
      });
    }
  } catch {
    // silently fail
...
function mapBasicTrack(raw: any): Track {
  return {
    id: raw.id,
    name: raw.name,
