...
let lyricsMode: 'original' | 'bilingual' | 'ruby' = 'bilingual';

export function renderPlayerPage(): void {
...
function createLyricsPanel(): HTMLElement {
  const panel = document.createElement('div');
  ...
  toolbar.querySelectorAll('button').forEach((btn) => {
    btn.addEventListener('click', () => {
      lyricsMode = (btn.dataset.lyricMode || 'bilingual') as 'original' | 'bilingual' | 'ruby';
...
function bindControls(page: HTMLElement): void {
  // Play/Pause
  ...
  // Mode
  page.querySelector('[data-action="mode"]')?.addEventListener('click', function (this: HTMLElement) {
  ...
  // Progress slider
  const slider = page.querySelector('#pp-slider') as HTMLInputElement;
...
function bindEvents(page: HTMLElement): void {
  bus.on('player:track-change', (track: Track) => updateTrackDisplay(page, track));
  bus.on('player:time-update', (time: number) => updateTimeDisplay(time));
  bus.on('player:state-change', (playing: boolean) => {
...
function updateTrackDisplay(page: HTMLElement, track: Track): void {
  ...
  if (totalEl) totalEl.textContent = formatTime(track.duration / 1000);
...
function updateTimeDisplay(time: number): void {
  ...
  const slider = document.getElementById('pp-slider') as HTMLInputElement;
...
// ---- Lyrics rendering ----
let currentLyricLines: HTMLElement[] = [];
...
function toRuby(text: string, romaText?: string): string {
  if (!romaText) return text;
...
function highlightLyricLine(time: number): void {
  const lyric = lyricsEngine.getCurrent();
  if (!lyric) return;
...
async function loadSimilar(songId: number): Promise<void> {
  try {
    ...
    if (metaSimilar && songs.length > 0) {
      metaSimilar.innerHTML = songs.slice(0, 5).map((s: any) =>
        `<div class="meta-row">
          <span>${s.name}</span>
          <span class="text-muted">${(s.artists||s.ar||[]).map((a:any)=>a.name).join('/')}</span>
        </div>`
      ).join('');
    }
  } catch {
