...
type SubPage = 'main' | 'playback' | 'ai-translate' | 'appearance' | 'about';
let activeSubPage: SubPage = 'main';
...
function renderMain(container: HTMLElement): void {
  ...
  const settSection = acctSection('设置', [
    ...
    acctItemInline('清除缓存', '', () => {
      Object.keys(localStorage).forEach(k => { if (k.startsWith('lyric_tr_')) localStorage.removeItem(k); });
      alert('缓存已清除');
    }),
  ]);
...
// ============ Sub-pages ============
function showSubPage(container: HTMLElement, page: SubPage): void {
  activeSubPage = page;
  container.innerHTML = '';
  ...
  switch (page) {
    case 'playback': renderPlaybackSub(container); break;
    case 'ai-translate': renderAISub(container); break;
    case 'appearance': renderAppearanceSub(container); break;
    case 'about': renderAboutSub(container); break;
  }
}

function renderPlaybackSub(container: HTMLElement): void {
...
function renderAISub(container: HTMLElement): void {
  ...
  fields.querySelector('#ai-key')?.addEventListener('change', (e) => localStorage.setItem('purify_ai_key', (e.target as HTMLInputElement).value));
  fields.querySelector('#ai-endpoint')?.addEventListener('change', (e) => localStorage.setItem('purify_ai_endpoint', (e.target as HTMLInputElement).value));
  fields.querySelector('#ai-model')?.addEventListener('change', (e) => localStorage.setItem('purify_ai_model', (e.target as HTMLInputElement).value));
}

function renderAppearanceSub(container: HTMLElement): void {
...
function renderAboutSub(container: HTMLElement): void {
...
function acctSection(title: string, items: HTMLElement[]): HTMLElement {
...
function acctItem(label: string, value: string, onClick: () => void): HTMLButtonElement {
...
function acctItemInline(label: string, value: string, onClick: () => void): HTMLButtonElement {
  const btn = document.createElement('button');
  btn.className = 'acct-item';
  btn.innerHTML = `
    <span class="acct-item-label">${label}</span>
...
function acctToggle(label: string, checked: boolean, onChange: (v: boolean) => void): HTMLElement {
  const row = document.createElement('div');
  row.className = 'acct-item';
...
async function fetchUserStats(): Promise<void> {
  try {
    ...
    if (d) setStat('as-playlists', String(d.createdPlaylistCount || 0));
    ...
    if (ids) setStat('as-likes', String(ids.length));
  } catch { /* silent */ }
}

function setStat(id: string, val: string): void {
  const el = document.getElementById(id);
  if (el) el.textContent = val;
}

function applyTheme(dark: boolean): void {
  const vars: [string, string][] = dark
