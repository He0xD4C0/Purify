...
// ============ Logged-in view ============
function renderLoggedIn(container: HTMLElement): void {
...
// ============ Helpers ============
function menuItem(icon: string, label: string, cls: string, onClick?: () => void): HTMLButtonElement {
...
async function fetchUserStats(): Promise<void> {
  try {
    ...
    if (detailData?.level != null) {
      ...
      if (levelEl) levelEl.textContent = `Lv.${detailData.level}`;
    }
