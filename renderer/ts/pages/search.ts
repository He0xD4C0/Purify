...
import { playTrack, type Track } from '../core/app.js';
import { renderSongList, type SongListTrack } from '../components/song-list.js';
import type { PlaylistInfo } from '../components/playlist-card.js';
...
async function fetchResults(container: HTMLElement, query: string, type: number): Promise<void> {
  ...
  try {
    ...
    switch (type) {
      case 1: renderSongResults(container, result.songs || [], query); break;
      case 10: renderAlbumResults(container, result.albums || []); break;
      case 100: renderArtistResults(container, result.artists || []); break;
      case 1000: renderPlaylistResults(container, result.playlists || []); break;
    }
  } catch {
...
// ---- Song results ----
function renderSongResults(container: HTMLElement, songs: any[], query: string): void {
  if (songs.length === 0) {
