...
export interface PlayabilityResult {
  canPlay: boolean;        // whether the song can be played at all
  status: MusicStatus;     // badge type for UI
  reason: string;          // human-readable reason if can't play
  maxQuality: string;      // max quality available: 'standard'|'higher'|'exhigh'|'lossless'|'hires'
}
...
export function checkPlayability(meta: SongMeta): PlayabilityResult {
  ...
  if (!priv) {
    if (fee === 0) return ok('free', 'standard');
    if (fee === 1) return userLoggedIn ? ok('vip', 'exhigh') : blocked('vip', 'VIP 限定，请登录后播放');
    if (fee === 4) return blocked('purchase', '付费单曲，需购买后播放');
    if (fee === 8) return userLoggedIn ? ok('free', 'standard') : blocked('free', '登录后可播放');
    if (fee === 16) return userLoggedIn && userVip === 'svip' ? ok('svip', 'lossless') : blocked('svip', '黑胶 SVIP 限定');
    return ok('free', 'standard');
  }

  const pl = priv.pl as number;
  const st = priv.st as number;
  const maxbr = priv.maxbr as number || 0;
  const freeTrial = priv.freeTrialInfo as unknown;

  // Status -200 = unavailable
  if (st === -200) return blocked('unavailable', '平台无版权');

  // pl === 0 and dl === 0: cannot play
  if (pl === 0 && (priv.dl as number || 0) === 0) {
  ...
  if (pl > 0) {
    if (fee === 0) return ok('free', quality);
    if (fee === 1) {
      if (!userLoggedIn) return blocked('vip', 'VIP 限定，请登录后播放');
      return ok('vip', quality);
    }
    if (fee === 4) return blockOrOk('purchase', '付费单曲，需购买后播放', quality);
    if (fee === 8) return ok(userLoggedIn ? 'free' : 'free', quality);
    if (fee === 16) {
      if (!userLoggedIn) return blocked('svip', '黑胶 SVIP 限定');
      if (userVip !== 'svip') return blocked('svip', '黑胶 SVIP 限定');
...
export function canPlayQuality(meta: SongMeta, requestedQuality: string): boolean {
  ...
  if (reqLevel >= qualities.indexOf('lossless')) {
    if (userVip === 'none') return false;
  }

  // hires requires SVIP
  if (requestedQuality === 'hires' && userVip !== 'svip') return false;
...
function ok(status: MusicStatus, quality: string): PlayabilityResult {
...
function blockOrOk(status: MusicStatus, reason: string, quality: string): PlayabilityResult {
  // purchase: show as blocked but let user know they can buy
  return { canPlay: false, status, reason, maxQuality: quality };
}

function maxbrToQuality(maxbr: number): string {
  if (maxbr >= 999000) return 'hires';
  if (maxbr >= 320000) return 'lossless';
  if (maxbr >= 192000) return 'exhigh';
  if (maxbr >= 128000) return 'higher';
  return 'standard';
}
