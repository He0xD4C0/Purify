...
export function renderLoginPanel(container: HTMLElement, onClose?: () => void): void {
  ...
  function renderPhoneForm(): void {
    ...
    const submitBtn = contentEl.querySelector('#login-submit') as HTMLButtonElement;
    submitBtn.addEventListener('click', async () => {
      const phone = (contentEl.querySelector('#login-phone') as HTMLInputElement).value;
      const password = (contentEl.querySelector('#login-password') as HTMLInputElement).value;
  ...
  // ---- QR form ----
  async function renderQrForm(): Promise<void> {
    contentEl.innerHTML = `
    ...
    try {
      ...
      if (!unikey) throw new Error('获取二维码失败');
