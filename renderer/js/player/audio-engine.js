...
class AudioEngine {
    ...
    async play(track) {
        ...
        // Get audio URL
        let url = this.urlCache.get(track.id)?.url;
    ...
    async fetchAudioUrl(track) {
        try {
            const res = await fetch(`http://${location.hostname}:15678/song/url/v1?id=${track.id}&level=lossless`, { method: 'POST' });
